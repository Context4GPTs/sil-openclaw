...
/** `dist/lib/openclaw-allowlist.js` as the compiler saw it, per call. */
...
function fakeCompile(argv: string[]): void {
  ...
  outDirs.push(outDir);
  const seen = join(distDir, "lib", "openclaw-allowlist.js");
...
function seedPreviousBuild(): void {
  ...
  writeFileSync(join(distDir, "lib", "openclaw-allowlist.js"), "// STALE\n");
}
...
describe("buildDist — compiles to a temp outDir and installs by rename only", () => {
  ...
  it("throws instead of installing when the build misses an entry a bin imports", () => {
    ...
    expect(readFileSync(join(distDir, "lib", "openclaw-allowlist.js"), "utf8")).toBe("// STALE\n");
...
describe("distEntriesTheBinsImport — derived from the bins, never a hand-list", () => {
  it("names every ../dist/*.js an operator script statically imports, plus the plugin entry", () => {
    ...
    // Anti-vacuity: an empty derivation would satisfy the equality above while
    // checking nothing, and this is the import that actually broke.
...
describe("the default install target is the directory the bins actually import", () => {
  it("DIST_DIR is where `../dist/lib/openclaw-allowlist.js` resolves from scripts/", () => {
    ...
    const asTheBinResolvesIt = resolve(REPO_ROOT, "scripts", "../dist/lib/openclaw-allowlist.js");
    expect(join(DIST_DIR, "lib", "openclaw-allowlist.js")).toBe(asTheBinResolvesIt);
  });
});
