...
// ---------------------------------------------------------------------------
// B1 (behaviour half) — sil_doctor performs EXACTLY the writes its description
// enumerates, and no others.
//
// The unit tier (`tools/doctor.test.ts`) pins what the description SAYS: it
// creates the missing data directory and narrows an over-permissive mode inside
// it. This block pins that the code does nothing beyond that pair — otherwise
// the honest description would be honest about the wrong thing. The existing
// AC4 block proves "no writes on a HEALTHY store"; this is its unhealthy
// counterpart, where writes are expected and must be exhaustively accounted for.
// ---------------------------------------------------------------------------

/** Entries whose recorded state differs between two snapshots. */
function snapshotDelta(
  before: Record<string, string>,
  after: Record<string, string>,
): { added: string[]; removed: string[]; changed: Array<[string, string, string]> } {
  const added = Object.keys(after).filter((k) => !(k in before));
  const removed = Object.keys(before).filter((k) => !(k in after));
  const changed: Array<[string, string, string]> = [];
  for (const k of Object.keys(before)) {
    if (k in after && after[k] !== before[k]) changed.push([k, before[k]!, after[k]!]);
  }
  return { added, removed, changed };
}

describe("B1 — sil_doctor's ONLY writes are the two its description enumerates", () => {
  it.skipIf(AS_ROOT)("on an over-permissive store, changes ONLY modes — never one byte", async () => {
    seedHealthyStore();
    seedHealthyDomain("coffee");
    chmodSync(dataDir, 0o755);
    chmodSync(shopperDir(), 0o755);
    chmodSync(join(domainDir("coffee"), "method.md"), 0o644);

    const before = snapshot(parentDir);
    await runDoctor();
    const after = snapshot(parentDir);
    const { added, removed, changed } = snapshotDelta(before, after);

    // Write #1 (dir creation) is not in play — the dir already existed.
    expect(added, "sil_doctor created something it never declared").toEqual([]);
    expect(removed, "sil_doctor deleted something — it deletes nothing").toEqual([]);
    expect(changed.length, "an over-permissive store produced no fix at all").toBeGreaterThan(0);

    for (const [path, from, to] of changed) {
      const [kindBefore, modeBefore, hashBefore] = from.split(":");
      const [kindAfter, modeAfter, hashAfter] = to.split(":");
      expect(kindAfter, `${path}: entry kind changed`).toBe(kindBefore);
      expect(hashAfter, `${path}: CONTENT changed — the doctor rewrote bytes`).toBe(hashBefore);
      // Write #2 only ever NARROWS: the new mode may not set a bit the old one
      // lacked. `to & ~from === 0`.
      const oldMode = Number(modeBefore);
      const newMode = Number(modeAfter);
      expect(
        newMode & ~oldMode,
        `${path}: mode ${oct(oldMode)} → ${oct(newMode)} GRANTED a bit`,
      ).toBe(0);
    }
  });

  it.skipIf(AS_ROOT)("touches nothing outside $SIL_DATA_DIR", async () => {
    // The sibling file lives in the parent dir, next to the store — the blast
    ...
    for (const key of new Set([...Object.keys(before), ...Object.keys(after)])) {
      ...
      // relpath does not start with its basename is outside it.
      const store = relative(parentDir, dataDir);
      const inStore = key === store || key.startsWith(`${store}/`);
      if (!inStore) {
        expect(after[key], `${key} changed outside the store`).toBe(before[key]);
      }
    }
  ...
  it("creates the missing data directory — write #1, and nothing more", async () => {
    rmSync(dataDir, { recursive: true, force: true });
    expect(existsSync(dataDir)).toBe(false);
    const sibling = join(parentDir, "sibling.txt");
    writeFileSync(sibling, "untouched", { mode: 0o644 });
    ...
    // The ONLY new entry is the store root itself; the writability probe's tmp
    // file must be gone by the time execute() returns.
    expect(added).toEqual([relative(parentDir, dataDir)]);
    expect(removed).toEqual([]);
    expect(changed.map(([k]) => k)).toEqual([]);
  });
});
