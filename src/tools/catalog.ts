...
export const SHOPPING_TOOLS = [
  ...
  },
  {
    ...
    description:
      ...
      + " the buyer states goes on the brief. A variant with no option values is a listing"
      + " whose sizes sil has not read: send its id like any other, priced as its page"
      + " prints. An offer is the pick at one shop, read live, from the shops sil holds and"
      + " ones it finds on the web: the seller's name and id, the price as the page prints"
      + " it in its own currency (sil converts nothing), its availability, the listing URL,"
      + " `observed_at`, which dates the price, and `seller_fit`: `ships` always —"
      + " serviceable, not_serviceable or unknown for that address — and each brief seller"
      + " key with that seller's value if sil holds one. `unknown` keeps the offer: say sil"
      + " could not confirm shipping and hand over the listing. A key absent from"
      + " `seller_fit` is a term sil has not read, never one the seller lacks. The order"
      + " is the buyer's currency and market first, then sellers known to reach them —"
      + " keep it, never re-rank. Several offers on one variant ARE the price spread, and"
      + " the spread is the answer — never one best.",
  },
  {
