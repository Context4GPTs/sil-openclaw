...
When the session is running **inside a created expert** (a loaded profile under `$SIL_DATA_DIR/agents/<agentId>/`) and the user states a shopping intent, follow [`references/expert_shopping.md`](references/expert_shopping.md) — the profile-driven, **Spec-Driven Shopping (SDS)** shop-time loop that layers the request, the persisted user spec, and the researched domain spec. It is additive: a plain, profile-less session keeps shopping via the "find X" → `sil_search` row above, unchanged.
