...
**What it remembers, and where it lives.** Your shopper learns your shopping-relevant facts as it goes — measurements, budget band, the rules it must never break, brand likes/dislikes — captured **only when a request actually needs them** (never a big up-front form) so it doesn't re-ask. All of it is stored **locally on your machine** (`$SIL_DATA_DIR/shopper/`, files owner-only `0600`, directories `0700`), **per-user (one shopper, many domains)**, as plain markdown whose own frontmatter is the source of truth — the shared `user_spec.md` plus one folder per domain it has learned. It is **never pooled across users and never sent to a server** for training or aggregation. Exactly two things are **offline**: **creating** your shopper (no sign-in, no account, no network call) and **keeping** what it learns on disk. The rest is online by design, and worth being precise about: finding products calls sil-api over the network using your registered sil identity's stored session token, and learning a new niche reads the public web. Neither uploads what your shopper knows about you. Inspect exactly what it holds with `sil_profile_get`, and forget one domain it learned with `sil_profile_remove`.
...
| `sil_specs` | Canonicalize the coined spec **definitions** a domain invents for its niche, so two niches that mean the same thing say it the same way. Internal plumbing your agent calls while learning a niche — never surfaced to you. |

**Your shopper** — create one shopper, then manage the domains it learns

Everything here is local: no network call, no token read. Artefacts live under `$SIL_DATA_DIR/shopper/` as markdown whose own frontmatter is the source of truth — the shared `user_spec.md`, then one folder per domain holding that niche's `method.md` and its intent-keyed PRDs. (Your shopper's persona is its host workspace `SOUL.md`, not a sil artefact.)
...
| `sil_profile_remove` | Forget ONE domain — or just one of its PRDs. Your shopper, your shared facts, and every other domain survive. Destructive, so the agent confirms with you first; it never removes the shopper itself. |

**Diagnostics**

| Tool | What it does |
|---|---|
| `sil_doctor` | Check this install and hand back a machine-readable findings list: the data directory, file permissions, stored identity/token health, artefact readability, host wiring, and whether a newer sil plugin is published. It writes only two things, both inside `$SIL_DATA_DIR` — it creates that directory if it is missing, and tightens an over-permissive mode back to owner-only. Anything that could lose data is reported, never run. |
