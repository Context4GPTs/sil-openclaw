...
### Step 7: Update Skill Rules

After creating or updating a skill, update `.claude/hooks/skill-rules.json` to enable automatic skill activation. This file configures when the skill evaluation hook suggests the skill based on user prompts.

**For new skills**, add an entry to the `skills` object:

```json
{
  "skills": {
    "my-new-skill": {
      "description": "Brief description of what triggers this skill",
      "priority": 6,
      "triggers": {
...
**Trigger types** (see `.claude/hooks/skill-rules.schema.json` for full schema):
- `keywords` - Simple words to match (case-insensitive)
- `keywordPatterns` - Regex patterns for prompt matching
- `pathPatterns` - Glob patterns for file paths mentioned in prompt
- `intentPatterns` - Regex patterns to detect user intent (e.g., `(?:create|add).*(?:feature)`)
- `contentPatterns` - Patterns to match in code snippets
- `contextPatterns` - Phrases indicating conversation context
- `excludePatterns` - Patterns that prevent skill from matching
- `relatedSkills` - Skills to suggest alongside this one

**Priority levels** (1-10):
- 9-10: Critical skills (debugging, TDD, brainstorming)
- 7-8: Core workflow skills (hooks, react patterns)
- 5-6: Domain-specific skills (browser automation, skill creation)
- 3-4: Supporting skills (documentation, verification)

**Optional directory mappings**: If the skill is associated with specific directories, add to `directoryMappings`:

```json
{
  "directoryMappings": {
    ".claude/skills": "skill-creator",
    "src/my-feature": "my-new-skill"
  }
}
```
